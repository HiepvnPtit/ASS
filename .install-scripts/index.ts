...
(async () => {
  ...
  if (response.database === 'pg-mongo') {
    void removeRelationalResourceGeneration();
    void removeDocumentResourceGeneration();
    void removeDocumentPropertyGeneration();
    void removeRelationalPropertyGeneration();
  }

  if (response.database === 'mongo') {
    void removePostgreSql();
    void removeRelationalResourceGeneration();
    void removeRelationalPropertyGeneration();
    void removeAllDbResourceGeneration();
    void removeAllDbPropertyGeneration();
  }
  ...
  if (!response.isAuthFacebook) {
    void removeFacebookAuth();
  }

  if (!response.isAuthGoogle) {
    void removeGoogleAuth();
  }

  if (!response.isAuthApple) {
    void removeAppleAuth();
  }

  void removeInstallScripts();
  process.exit(0);
})();
